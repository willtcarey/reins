...
4. **User-provided documentation references** — Pi's default prompt includes paths to its own docs in node_modules so the agent can look things up. Reins should support a similar mechanism for user-specified docs (library docs, API references, internal guides). The challenge: with the node architecture, docs might live on the user's machine but the system prompt is assembled on the backend. The paths would need to be node-relative, or docs would need to be synced/registered as part of the project config rather than hardcoded paths.
