...
`review-virtual-layout.ts` now exposes one persistent `ReviewVirtualCoordinator` rather than a collection of stateless prefix-layout helpers. Stable `ReviewItem` IDs remain the source of identity, while an expanded measurement key adds project, branch, and content so valid dimensions persist and stale dimensions naturally stop applying. Initial heights still use Pierre's 20px row metric, each hunk's exact `unifiedLineCount`, line-info separator geometry, no-newline metadata rows, the Reins header, and in-box inter-file spacing.
