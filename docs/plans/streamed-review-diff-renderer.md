...
The `virtualized` renderer currently maps every review item into mounted DOM. It is intentionally a functional scaffold, not a performance solution. Its Reins-owned headers include accessible collapse controls plus the shared view, copy-path, and download actions. Collapsed state is keyed by stable review item ID, survives patch reconciliation for surviving records, and expands a target before file-tree navigation scrolls to it. Context expansion, per-file content retrieval, rich previews, streaming, and top-level virtualization remain follow-up work.
