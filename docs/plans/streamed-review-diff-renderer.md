...
5. [x] **Add Reins-owned behavior that `CodeView` cannot own cleanly.**
...
8. [x] **Implement fluid inline context expansion through Pierre `FileDiff`.**
   ...
   - Use Pierre's native `line-info` rows, partial-diff controls, `loadDiffFiles`, and `FileDiff.expandHunk` interface.
   - Lazily retrieve the bounded complete resulting file on the first native expansion, reverse-apply and validate the retained Git patch to reconstruct the old file, and share the request for that reviewed item.
   - Keep the partial `FileDiff` mounted while acquisition is pending or fails. Let Pierre hydrate its stable metadata in place, and retain public expansion commands only for unavoidable virtual remount restoration.
...
- Pierre 1.4.1 exposes native async partial-diff expansion through `loadDiffFiles`, including an unknown trailing-context control. Reins always passes truthful partial metadata, validates loaded contents against the retained Git patch, and lets Pierre hydrate that stable object in place. Mounting controls does not acquire content; native activation starts the shared request. Reins does not prefetch mounted or offscreen items, add synthetic lines, fake metadata completeness, use deprecated custom separators, or inspect protected expansion state.
...
`@pierre/diffs` 1.4.1 now owns partial-diff controls and async hydration through `loadDiffFiles`. `FileDiffContextState` supplies that loader, shares each bounded request, retains acquisition failures, and records only public `expandHunk` commands for virtual remount restoration. It delegates content loading to `loadFileContents`; before returning reconstructed old/new files to Pierre, `buildFileDiff` validates them against each `FileChange`'s exact Git patch, including multiple hunks, additions/deletions, zero-length ranges, and no-newline-at-EOF markers. Pierre upgrades the stable partial metadata in place and renders leading, inter-hunk, and unknown trailing controls natively. Reins no longer injects acquisition DOM, intercepts pointer activation, reads protected expansion state, or subclasses `FileDiff`. A small public-handler adapter still opens the outer virtual-list anchoring transaction, while the keyboard adapter makes Pierre's role-button controls operable with Enter and Space.
