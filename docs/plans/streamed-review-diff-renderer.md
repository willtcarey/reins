...
The `virtualized` renderer now keeps all review records in JavaScript while mounting only the viewport and a bounded pixel overscan window. Its Reins-owned headers include accessible collapse controls plus the shared view, copy-path, and download actions. A collapsed item records the hash of that exact reviewed diff in local storage under its project, branch, and stable item ID; both diff modes share that reviewed state. Matching content remains collapsed across reconciliation, project switches, and reloads; changed content expands and invalidates the marker so a later revert also stays expanded. File-tree navigation resolves an estimated item offset and therefore works before the target wrapper exists. Active-file tracking is derived from virtual geometry rather than mounted-element intersection. Resize measurements replace estimates for mounted states, and height changes above the viewport compensate `scrollTop` to retain the anchor. Context expansion, per-file content retrieval, rich previews, pooling, patch streaming, and performance measurement remain follow-up work.
...
The previously proposed stateful `setItems` / `scrollToItem` / `reportMeasuredHeight` / `getVisibleItems` manager was not adopted. It would duplicate ownership already present in `ReviewDiffPanel` and create a second imperative item lifecycle beside Lit.

Instead, `ReviewDiffPanel` remains the interaction coordinator and the existing stable `ReviewItem` records remain the source of identity. `review-virtual-layout.ts` is a small pure geometry module: the panel derives a prefix-offset layout from item IDs and estimated/measured heights, then derives the active record and bounded render window from a viewport snapshot. Navigation looks up the target offset in that same layout. Initial heights use Pierre's 20px row metric, each hunk's exact `unifiedLineCount`, line-info separator geometry, no-newline metadata rows, Reins header height, and in-box inter-file spacing. Measurements are held in a `WeakMap` keyed by reconciled `ReviewItem` identity, so unchanged records retain dimensions without adding mutable rendering state to the parser model. `ResizeObserver` feeds mounted dimensions back into the next derived layout, with scroll compensation when a changed item is wholly above the viewport. Expanded items are not measured until Pierre reports its non-placeholder diff render complete; caching the transient header-only height while worker rendering was in flight caused large late corrections after mobile momentum scrolling. Passive measurements for items wholly above the viewport are deferred until those items intersect instead of mutating `scrollTop` against an active upward gesture. The mount window overscans after, but not before, the viewport so unfinished worker renders cannot physically push the visible rows from above. Explicit collapse changes still preserve the current geometry anchor. Native CSS scroll anchoring is disabled on this container so it does not apply a second competing correction.

This shape fits the existing seams: parsing/reconciliation owns records, the panel owns navigation/collapse/active state, Lit owns mounting, and `ReviewDiffItem` owns a Pierre `FileDiff` only for its mounted lifetime. It also leaves room for future mixed item kinds without exposing a generic virtualizer API prematurely.
