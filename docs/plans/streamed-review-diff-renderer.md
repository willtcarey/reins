...
The generic virtual list remains the geometry owner. Before a native separator interaction, the item records the separator and following line point; after Pierre renders, it submits that point delta to `VirtualListController`. The controller waits for the stable `ResizeObserver` measurement to update persistent item/total height, then applies the point compensation against the new virtual geometry. This prevents bottom clamping against stale total height and keeps both upward and downward reveals anchored; intervening user scroll intent cancels the pending correction.
