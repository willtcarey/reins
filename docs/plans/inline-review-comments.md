...
A detached backend `CodeReview` entity owns annotation creation, review-wide source-key upsert, and replies. SQLite persists its project/task/revision envelope and annotation aggregate with optimistic compare-and-swap. The frontend `CodeReviewStore` loads the exact session scope, sends comment intents with the current review identity/revision, reloads on reconnect and newer matching `code_review_updated` invalidations, and is the sole publisher of authoritative raw review state. Saved comments return after browser refresh when their canonical selected diff rows still match the rendered diff; Pierre side/range coordinates are derived only at the rendering seam, while unsaved composer text remains frontend-local. Review submission is now implemented for the virtualized renderer: a floating action appears only when saved comments exist, targets the selected idle session, and `CodeReviewSubmission` atomically replaces the pending review with one compiled ordinary user message. This slice still has no outdated-thread surface, Git/filesystem relocation, comment editing, or supported plugin SDK. The internal agent scripting interface can read the current review and add a line/range comment while deriving its anchor from the current Git diff. Saved comments can be deleted before submission through the Reins-owned optimistic-concurrency route.
...
interface ReviewAnchor {
  ...
  side: ReviewSide;              // selected split-view side
  startLine: number;             // original coordinate; end derives from row count
  lines: ReviewDiffLine[];       // canonical selected row contents

  itemContentFingerprint: string;// compact identity for the reviewed per-file patch
  filePatch: string;             // exact Git-native per-file patch shown at creation
...
3. **Selected diff rows are canonical.** Persist the selected `old/new` side, original start coordinate, and each row's kind and text; never persist Pierre's `deletions/additions` vocabulary. Same-side contiguity makes per-row old/new coordinates redundant.
4. **Pierre coordinates are a projection.** Derive the end coordinate from the start and selected row count, and derive any relocated side/range only at the adapter seam.
5. **Selected rows own current placement.** On the same path lineage, first match the complete rows at their recorded start coordinate. If coordinates moved, relocate only when one contiguous rendered sequence has the same ordered kinds and text.
6. **Missing, changed, or ambiguous rows are outdated.** Do not guess among duplicate matches; retain the annotation and its original diff evidence for submission and a future outdated surface.
7. **Submission uses the same evidence.** Retain the exact Git-native per-file patch shown when the comment is saved and include it verbatim (apart from message indentation) beneath the path. Do not reconstruct patch lines from normalized rows; native file, mode, index, and hunk headers are part of the review context.
...
- Stored anchor evidence includes path lineage, the selected side needed for split-context ambiguity, ordered structured diff rows, the exact Git-native per-file patch displayed at creation, and file/revision hints. Actual filesystem/Git matching remains future integration work and produces a transient exact, relocated, or unanchored submission resolution.
- Submission payload compilation belongs to the server submission mediator, not this entity. The mediator groups comments by their canonical selection and emits the saved native per-file patch beneath the path without synthesizing patch markers or headers; later comments at the same selection are rendered as replies.
...
`ProjectCodeReviews`, exposed through `ProjectModel.codeReviews()`, is the reusable project-scoped model for REST and the agent scripting adapter. It owns task-scope validation, exact scoped review resolution, atomic pending-review find/create, optimistic concurrency, persistence, anchor construction, generated identities/timestamps, and post-commit invalidation. The detached `CodeReview` owns only annotation invariants and mutations. `code-review.ts` is the canonical source for code-review value types and their TypeBox schemas; domain types are derived from those schemas, and REST plus scripting adapters reuse the same contracts instead of redeclaring them. REST exposes exact-scope retrieval, comment creation/deletion, and submission resources; omitting `taskId` selects project scope. Comment creation accepts only path, selected range, body, and the exact displayed patch plus an optional review identity/revision guard; persisted annotation details are server-owned. `CodeReviewSubmission` owns submission eligibility, annotation grouping and message formatting, destination-session scope and idle checks, atomic message insertion plus pending-review deletion, and runtime dispatch. A retry cannot duplicate feedback because the referenced review no longer exists after acceptance, though it returns a conflict rather than replaying a receipt. Each committed review mutation broadcasts a scoped `code_review_updated` invalidation. Future Git reconciliation belongs in `CodeReviewSubmission`.
...
Frontend ownership has two state owners: `CodeReviewStore` synchronizes raw `CodeReviewState`, while the panel-lifetime `InlineReviewController` owns one selection and unsaved composer. The frontend keeps only the pure saved-anchor placement projection; canonical anchor construction lives in the backend and is shared by REST and scripting. There is no frontend aggregate wrapper, second subscription mechanism, command bus, per-file action map, or stateful annotation element.
...
`CodeReviewStore` owns synchronized raw state and accepts one `addComment({ path, side, startLine, endLine, filePatch, body })` intent. Pure `reviewPlacements(review, file)` contains the frontend's only durable projection rule; the backend turns comment intents into canonical anchor evidence.
...
- **Line number/path as the only durable evidence:** coordinates alone cannot distinguish changed or recreated diff rows. Persist the selected structured rows and derive coordinates for Pierre.
...
- [x] Render one grouped Reins-owned annotation host per side/endpoint with one in-memory composer and persisted threads.
- [x] Allow each saved comment to be deleted before submission through a revision-guarded persistence route.
...
- [x] Add discoverable `api.reviews.current()` and `api.reviews.addComment()` agent scripting operations that derive anchors from the current Git diff; keep supported external plugin loading/versioning out of this internal seam.
...
- A saved comment is placed when its canonical selected diff rows match exactly or have one unambiguous contiguous relocation; changed, absent, or ambiguous rows remain saved for submission and a future outdated surface.
