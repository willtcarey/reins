...
The backend mediator exposes project-scratch `GET /api/projects/:id/code-review` and `POST /api/projects/:id/code-review/annotations` routes alongside the exact task-scoped `GET /api/projects/:id/tasks/:taskId/code-review` and `POST /api/projects/:id/tasks/:taskId/code-review/annotations` routes. The POST accepts a client-generated annotation and entry identity plus an optional known `reviewId`/`revision` pair. Without a known pair it atomically finds or creates the one open exact-scope review; with a known pair, missing, mismatched-scope, genuinely stale mutations, or terminal reviews conflict rather than switching scope/history. An exact retry of the same client identities and content against the current open review is idempotent even when it repeats the now-stale revision used by the original committed mutation; it returns the current authoritative aggregate without saving or rebroadcasting, while identity reuse with different content conflicts. The committed workflow result reports whether it created the review so REST can return `201` without a pre-transaction existence read; existing/retried reviews return `200`. Each successful committed mutation broadcasts a scoped `code_review_updated` notification containing only project ID, nullable task ID, review ID, revision, and status. Frontend stores apply only newer revisions and reload the authoritative aggregate when the notification matches their active scope; initial load and WebSocket reconnect still use REST because broadcasts are not durable. A future cross-module submission workflow may coordinate Git reconciliation, sessions, messages, and broadcasts before marking and saving the review, but that orchestration must not move annotation or lifecycle rules out of the detached entity. No such submission payload is part of this slice.
...
- [x] Add exact project/task-scoped REST load and annotation mutation routes, an atomic backend mediator, optimistic conflicts, idempotent client-identity retry, and post-commit invalidation broadcasts.
