...
- `tsc --noEmit` reports 35 errors in `packages/frontend`. Most are pre-existing type mismatches in tests (`chat-panel-dedup`, `diff-file-card-markdown`, `project-store`) and a few component issues (`createRenderRoot` override signatures, class decorator resolution in `project-form.ts`, `task-detail.ts`). The `experimentalDecorators` fix (now in tsconfig) cleared the bulk (370 → 35), but the remainder should be fixed so `tsc` can run clean in CI.
