...
- Two parallel resource loaders. The PI runtime uses `DefaultResourceLoader` from `@earendil-works/pi-coding-agent` (wired up in `runtimes/pi/factory.ts` and fed back into the system prompt via `systemPromptOverride`), while the Claude SDK adapter uses our own `ReinsResourceLoader` (in `runtimes/resource-loader.ts`). Both discover the same standard locations (`~/.agents/skills`, `<project>/.agents/skills`, AGENTS.md ancestry) but independently, so `<available_skills>` on the PI side comes from pi's loader and on the Claude SDK side from ours. Unify by having the PI path consume `ReinsResourceLoader` outputs (pass discovered skills/context files through `buildReinsSystemPrompt` the way the Claude SDK adapter does) so there's a single source of truth for what Reins considers a "skill" or "context file". This matters especially now that the slash-command expansion layer uses `ReinsResourceLoader` — any drift between the two loaders means the model's `<available_skills>` listing on PI could disagree with what `/name` expansion can match.
...
- Legacy component files listed in the `reins/no-custom-event-constructor` override in `.oxlintrc.json` still instantiate `CustomEvent` inline. Move those constructors into `components/events.ts` as typed factory functions and remove each file's lint exemption as it is migrated.
