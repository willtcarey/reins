...
- Inline context expansion records the interaction before rendering and submits it to the existing controller correction path. Pierre names its visually upward/from-end reveal direction `down`; that operation scrolls down by the expanded item's measured height growth, keeping the following reviewed code in place even when Pierre leaves the separator fixed. Pierre's visually downward/from-start direction is `up`; it leaves scroll position unchanged because lines are inserted below the preceding reviewed code. The controller applies any correction only after the new item height updates virtual geometry, avoiding stale-total-height clamping; user scroll intent cancels a pending correction.
