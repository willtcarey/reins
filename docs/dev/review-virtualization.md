...
- Pure functions in `models/code-review.ts` project synchronized `CodeReviewState` into placements and build anchor evidence. `InlineReviewController.file(id)` is the single per-file interface passed through the diff UI; Pierre annotation elements receive only a placement whose composer owns its actions.
