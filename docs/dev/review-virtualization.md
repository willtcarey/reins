...
- Inline context expansion records the interacted Pierre separator and following changed line before rendering, then submits the selected point delta to the existing controller correction path. Upward expansion anchors the following changed line so revealing lines above it scrolls down; other directions retain the separator where possible. The controller applies the delta only after the expanded item's new height updates virtual geometry, avoiding stale-total-height clamping; user scroll intent cancels a pending correction.
