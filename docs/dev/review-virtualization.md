...
- `ReviewDiffPanel` is the review adapter. It owns patch reconciliation and height estimates, maps collapse to fixed geometry, resolves paths to item IDs, maps generic observations to review active-file events and telemetry, and coordinates store refreshes. Its `InlineReviewController` owns normalized selection and the single unsaved composer for the panel lifetime, so that ephemeral interaction survives child remounts without becoming a notifying store.
- `models/changes/review-diff-anchor.ts` translates Pierre metadata into Reins-owned structured diff rows. Pure functions in `models/code-review.ts` persist those selected rows as canonical placement evidence together with the exact Git-native `FileChange.filePatch` used for submission, and derive Pierre side/range placements from an exact coordinate match or one unique contiguous kind/text relocation. `InlineReviewController.file(id)` is the single per-file interface passed through the diff UI; Pierre annotation elements receive only a placement whose composer owns its actions.
...
- Pierre owns diff rows, native context-expansion regions, async hydration, and worker-backed highlighting inside the current container. `FileDiffContextState` supplies Pierre's `loadDiffFiles` callback, shares bounded lazy acquisition, and retains public expansion commands across virtual remounts. `loadFileContents` acquires the complete resulting file, while the patch module reconstructs and validates the old file from the retained Git patch before Pierre may hydrate its stable partial metadata in place. New/deleted files already have complete one-sided patch contents. Pierre renders native controls for leading, inter-hunk, and unknown trailing context without fetching merely because a bounded control mounted. The first activation starts acquisition; remounts replay only public `expandHunk` commands and attach to any in-flight load.
...
- Files that remain in overlapping windows retain their component and renderer. Collapsing unmounts only the renderer; expansion rebuilds its target and replays retained public expansion commands before first render so the cached expanded height still matches the restored body.
