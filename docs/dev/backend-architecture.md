...
- `models/workspace.ts` — checkout-scoped workspace behavior, including rooted working-tree/Git file streams, changed-file summaries, raw patch streams, parsed diff DTOs, and orchestration of temporary indexes for untracked-file diffing. File routes must obtain content through this model rather than accessing the checkout directly.
