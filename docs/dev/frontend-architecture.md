...
- **CodeReviewStore** (`models/stores/code-review-store.ts`) — The authoritative current-review aggregate for the viewed session's exact project/task scope. It owns initial/reconnect loads, optimistic annotation writes, review submission progress/errors, and revision-aware WebSocket invalidation reloads. Submission explicitly receives the selected session identity at action time; runtime activity remains owned by `SessionCache` and is passed to the virtualized review header as policy input. `ReviewComments` adapts the aggregate to path/fingerprint-validated Pierre placements while retaining unsaved composers locally.
