...
- `virtualized-review-items.ts` — Parses raw patches into stable Reins-owned review item identities, Pierre cache keys, and path-to-item navigation records.
...
- `virtualized-diff-panel.ts` — Reins-owned scaffold with item records, minimum file headers, item-ID navigation/active reporting, and Pierre worker-backed text rows. Despite the renderer setting name, this scaffold currently mounts every item and is not a performance solution.
